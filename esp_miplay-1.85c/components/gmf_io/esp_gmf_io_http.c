...
static esp_gmf_err_t _http_open(esp_gmf_io_handle_t self)
{
    ...
    /* 自定义 User-Agent（QQ 音乐 CDN 拒绝默认的 ESP32 HTTP Client/1.0） */
    if (http_io_cfg->user_agent) {
        esp_http_client_set_header(http->client, "User-Agent", http_io_cfg->user_agent);
    }
...
static esp_gmf_err_io_t _http_acquire_read(esp_gmf_io_handle_t handle, void *payload, uint32_t wanted_size, int block_ticks)
{
    ...
    if (rlen > 0) {
        ...
        http->connect_times++;
        /* 退避延迟：第 1 次 500ms，第 2 次 1s，第 3 次 2s，第 4 次 4s */
        int delay_ms = 500 << (http->connect_times - 1);
        if (delay_ms > 4000) delay_ms = 4000;
        ESP_LOGW(TAG, "Reconnect attempt %d/%d (backoff %d ms)",
                 http->connect_times, HTTP_MAX_CONNECT_TIMES, delay_ms);
...
esp_gmf_err_t esp_gmf_io_http_set_user_agent(esp_gmf_io_handle_t handle, const char *user_agent)
{
    ESP_GMF_NULL_CHECK(TAG, handle, return ESP_GMF_ERR_INVALID_ARG;);
    http_io_cfg_t *http_io_cfg = (http_io_cfg_t *)OBJ_GET_CFG(handle);
    http_io_cfg->user_agent = user_agent;
    return ESP_GMF_ERR_OK;
}
