...
#define HTTP_STREAM_BUFFER_SIZE      (16 * 1024)
...
typedef struct {
    ...
    const char             *user_agent;          /*!< Custom User-Agent string, NULL for default */
...
#define HTTP_STREAM_CFG_DEFAULT()  {                     \
    ...
    .io_cfg       = {                                    \
        .thread = {                                      \
            .stack        = HTTP_STREAM_TASK_STACK,      \
            .prio         = HTTP_STREAM_TASK_PRIO,       \
            .core         = HTTP_STREAM_TASK_CORE,       \
            .stack_in_ext = true,                        \
        },                                               \
        .buffer_cfg = {                                  \
...
/**
 * @brief  Set custom User-Agent for HTTP requests
 *
 * @note   If not set, default "ESP32 HTTP Client/1.0" is used.
 *         The string is deep-copied; caller does not need to keep it alive.
 *
 * @param  handle      The http IO handle
 * @param  user_agent  The User-Agent string (can be NULL to use default)
 *
 * @return
 *       - ESP_GMF_ERR_OK           on success
 *       - ESP_GMF_ERR_INVALID_ARG  Invalid argument
 */
esp_gmf_err_t esp_gmf_io_http_set_user_agent(esp_gmf_io_handle_t handle, const char *user_agent);
