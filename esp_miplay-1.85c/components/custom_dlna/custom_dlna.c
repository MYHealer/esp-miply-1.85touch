...
#define CUSTOM_DLNA_SSDP_STACK_BYTES        (16U * 1024U)
#define CUSTOM_DLNA_SSDP_ALIVE_STACK_BYTES  (12U * 1024U)
#define CUSTOM_DLNA_HTTPD_STACK_BYTES       (32U * 1024U)
#define CUSTOM_DLNA_GENA_STACK_BYTES        (32U * 1024U)
#define CUSTOM_DLNA_POS_NOTIFY_STACK_BYTES  (16U * 1024U)

static void custom_dlna_delete_current_task(void)
{
...
static EXT_RAM_BSS_ATTR gena_subscriber_t s_subs[MAX_SUBSCRIBERS];
...
static void ssdp_alive_task(void *arg)
{
    (void)arg;
    int sock = socket(AF_INET, SOCK_DGRAM, 0);
    if (sock < 0) { ESP_LOGE(TAG, "SSDP alive socket failed"); custom_dlna_delete_current_task(); return; }
    int opt = 1;
...
static void ssdp_task(void *arg)
{
    (void)arg;
    int sock = socket(AF_INET, SOCK_DGRAM, 0);
    if (sock < 0) { ESP_LOGE(TAG, "SSDP socket failed"); custom_dlna_delete_current_task(); return; }
    int opt = 1;
    setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    ...
    if (bind(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        ESP_LOGE(TAG, "SSDP bind failed"); close(sock); custom_dlna_delete_current_task(); return;
    }
    struct ip_mreq mreq;
...
/* DLNA 网络任务只使用普通 CPU 内存，优先放到 PSRAM，给 lwIP 和 DMA
 * 保留内部 SRAM。PSRAM 不可用时保留内部栈回退，便于复用该组件。 */
static BaseType_t custom_dlna_create_task(TaskFunction_t task, const char *name,
                                          uint32_t stack_bytes, void *arg,
                                          UBaseType_t priority, BaseType_t core)
{
    BaseType_t ret = xTaskCreatePinnedToCoreWithCaps(
        task, name, stack_bytes, arg, priority, NULL, core,
    ...
    ret = xTaskCreatePinnedToCore(task, name, stack_bytes, arg,
                                  priority, NULL, core);
    if (ret != pdPASS) {
        ESP_LOGE(TAG, "Task %s creation failed (%u bytes)",
                 name, (unsigned)stack_bytes);
    }
    return ret;
}
...
esp_err_t custom_dlna_init(const custom_dlna_config_t *config)
{
    ...
    /* Start SSDP tasks */
    custom_dlna_create_task(ssdp_task, "ssdp",
                            CUSTOM_DLNA_SSDP_STACK_BYTES, NULL, 5, 0);
    custom_dlna_create_task(ssdp_alive_task, "ssdp_alive",
                            CUSTOM_DLNA_SSDP_ALIVE_STACK_BYTES, NULL, 4, 0);
    ...
    if (notify_queue) {
        BaseType_t r = custom_dlna_create_task(gena_task, "gena_notify",
                                               CUSTOM_DLNA_GENA_STACK_BYTES,
                                               NULL, 5, 0);
        if (r != pdPASS) {
    ...
    if (s_pos_notify_sem) {
        custom_dlna_create_task(pos_notify_task, "pos_notify",
                                CUSTOM_DLNA_POS_NOTIFY_STACK_BYTES,
                                NULL, 4, 0);
