...
static esp_gmf_job_err_t esp_gmf_alc_process(esp_gmf_element_handle_t self, void *para)
{
    ...
    if (!in_port || !out_port) {
        ESP_LOGW(TAG, "ALC process: port NULL (in=%p, out=%p), pipeline may be resetting", in_port, out_port);
...
esp_gmf_err_t esp_gmf_alc_set_gain_all(esp_gmf_element_handle_t handle, int8_t gain)
{
    ESP_GMF_NULL_CHECK(TAG, handle, {return ESP_GMF_ERR_INVALID_ARG;});
    esp_gmf_alc_t *alc = (esp_gmf_alc_t *)handle;
    esp_gmf_err_t ret = ESP_GMF_JOB_ERR_OK;
    esp_gmf_oal_mutex_lock(((esp_gmf_audio_element_t *)handle)->lock);
    for (uint8_t i = 0; i < alc->max_ch; i++) {
        if (alc->alc_hd) {
            esp_ae_err_t ae_ret = esp_ae_alc_set_gain(alc->alc_hd, i, gain);
            if (ae_ret != ESP_AE_ERR_OK) ret = ESP_GMF_JOB_ERR_FAIL;
        }
        alc->gain[i] = gain;
    }
    esp_gmf_oal_mutex_unlock(((esp_gmf_audio_element_t *)handle)->lock);
