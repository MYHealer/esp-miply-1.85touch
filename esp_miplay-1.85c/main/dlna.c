...
#define MIPLAY_MEDIA_QUEUE_LEN 8
static QueueHandle_t s_miplay_media_queue = NULL;
static StaticQueue_t s_miplay_media_queue_control;
static EXT_RAM_BSS_ATTR uint8_t s_miplay_media_queue_storage[
    MIPLAY_MEDIA_QUEUE_LEN * sizeof(miplay_media_event_t)];
static EXT_RAM_BSS_ATTR miplay_media_event_t s_miplay_media_event_work;
...
static EXT_RAM_BSS_ATTR char s_miplay_id[64];
static EXT_RAM_BSS_ATTR char s_miplay_audio_id[64];
static EXT_RAM_BSS_ATTR char s_miplay_title[128];
...
static TaskHandle_t   s_album_art_task = NULL;
...
static void dlna_delete_current_task(void)
{
...
static BaseType_t dlna_create_task(TaskFunction_t task, const char *name,
                                   ...
                                   BaseType_t core)
{
    ...
    ESP_LOGW(TAG, "Task %s PSRAM stack creation failed; trying internal stack",
             name);
    ret = xTaskCreatePinnedToCore(task, name, stack_bytes, arg,
                                  priority, handle, core);
    if (ret != pdPASS) {
        ESP_LOGE(TAG, "Task %s creation failed (stack=%u bytes)",
                 name, (unsigned)stack_bytes);
    }
    return ret;
}
...
static void schedule_delayed_stop_notify(void)
{
    ...
    if (!fa) {
        ESP_LOGW(TAG, "Finish notify arg alloc failed");
        return;
    }
    fa->generation = s_media_generation;
    if (dlna_create_task(delayed_stop_notify, "stop_dly",
                         DLNA_DELAYED_STOP_STACK_BYTES, fa,
                         5, NULL, 1) != pdPASS) {
        free(fa);
    }
}
...
static EXT_RAM_BSS_ATTR char s_cur_artist[128];
...
static int clamp_miplay_ms(int64_t value)
{
    if (value <= 0) return 0;
...
static int get_miplay_position_ms(void)
{
    int64_t position = s_miplay_position_ms;
    if (s_miplay_state == PS_PLAYING && s_miplay_position_anchor_us > 0) {
        position += (esp_timer_get_time() - s_miplay_position_anchor_us) / 1000LL;
    }
    if (s_miplay_duration_ms > 0 && position > s_miplay_duration_ms)
        position = s_miplay_duration_ms;
    return clamp_miplay_ms(position);
}

static void reset_miplay_media_locked(void)
{
    s_miplay_id[0] = '\0';
    s_miplay_audio_id[0] = '\0';
    s_miplay_title[0] = '\0';
    s_miplay_artist[0] = '\0';
    s_miplay_album[0] = '\0';
    s_miplay_cover_url[0] = '\0';
...
static void consume_miplay_media_events_locked(void)
{
    if (s_miplay_session_reset_pending) {
        s_miplay_session_reset_pending = false;
        reset_miplay_media_locked();
    }
    if (!s_miplay_media_queue) return;
    miplay_media_event_t *event = &s_miplay_media_event_work;

    /* 断开后只丢弃网络任务尚未消费的旧事件，不能把它带回普通 DLNA UI。 */
    if (!s_miplay_connected) {
        while (xQueueReceive(s_miplay_media_queue, event, 0) == pdTRUE) {}
        return;
    }

    while (xQueueReceive(s_miplay_media_queue, event, 0) == pdTRUE) {
        bool event_has_position = (event->changed & MIPLAY_MEDIA_CHANGED_POSITION) != 0;
        bool id_changed = (event->changed & MIPLAY_MEDIA_CHANGED_ID) &&
                          event->id[0] && strcmp(s_miplay_id, event->id) != 0;
        bool audio_id_changed = (event->changed & MIPLAY_MEDIA_CHANGED_AUDIO_ID) &&
                                event->audio_id[0] &&
                                strcmp(s_miplay_audio_id, event->audio_id) != 0;
        bool title_changed = (event->changed & MIPLAY_MEDIA_CHANGED_TITLE) &&
                             event->title[0] && strcmp(s_miplay_title, event->title) != 0;
        bool cover_changed = (event->changed & MIPLAY_MEDIA_CHANGED_COVER_URL) &&
                             event->cover_url[0] && s_miplay_cover_url[0] &&
                             strcmp(s_miplay_cover_url, event->cover_url) != 0;
        ...
        if ((event->changed & MIPLAY_MEDIA_CHANGED_ID) && event->id[0]) {
            strncpy(s_miplay_id, event->id, sizeof(s_miplay_id) - 1);
            s_miplay_id[sizeof(s_miplay_id) - 1] = '\0';
        }
        if ((event->changed & MIPLAY_MEDIA_CHANGED_AUDIO_ID) && event->audio_id[0]) {
            strncpy(s_miplay_audio_id, event->audio_id, sizeof(s_miplay_audio_id) - 1);
            s_miplay_audio_id[sizeof(s_miplay_audio_id) - 1] = '\0';
        }
        if ((event->changed & MIPLAY_MEDIA_CHANGED_TITLE) && event->title[0]) {
            strncpy(s_miplay_title, event->title, sizeof(s_miplay_title) - 1);
            s_miplay_title[sizeof(s_miplay_title) - 1] = '\0';
        }
        if ((event->changed & MIPLAY_MEDIA_CHANGED_ARTIST) && event->artist[0] &&
            (!lyric_as_title || !s_miplay_artist[0])) {
            strncpy(s_miplay_artist, event->artist, sizeof(s_miplay_artist) - 1);
            s_miplay_artist[sizeof(s_miplay_artist) - 1] = '\0';
        }
        if ((event->changed & MIPLAY_MEDIA_CHANGED_ALBUM) && event->album[0] &&
            (!lyric_as_title || !s_miplay_album[0])) {
            strncpy(s_miplay_album, event->album, sizeof(s_miplay_album) - 1);
            s_miplay_album[sizeof(s_miplay_album) - 1] = '\0';
        }
        bool cover_same = false;
        if ((event->changed & MIPLAY_MEDIA_CHANGED_COVER_URL) && event->cover_url[0]) {
            cover_same = s_miplay_cover_url[0] &&
                         strcmp(s_miplay_cover_url, event->cover_url) == 0;
            if (!cover_same) {
                strncpy(s_miplay_cover_url, event->cover_url, sizeof(s_miplay_cover_url) - 1);
                s_miplay_cover_url[sizeof(s_miplay_cover_url) - 1] = '\0';
            }
        }
        if ((event->changed & MIPLAY_MEDIA_CHANGED_DURATION) && event->duration_ms > 0)
            s_miplay_duration_ms = clamp_miplay_ms(event->duration_ms);
        if (event->changed & MIPLAY_MEDIA_CHANGED_POSITION) {
            s_miplay_position_ms = clamp_miplay_ms(event->position_ms);
            if (s_miplay_state == PS_PLAYING)
                s_miplay_position_anchor_us = esp_timer_get_time();
        }
        if (event->changed & MIPLAY_MEDIA_CHANGED_PLAYER_STATE) {
            int frozen_position = get_miplay_position_ms();
            s_miplay_state = event->player_state;
            if (s_miplay_state == PS_PLAYING) {
                s_miplay_position_anchor_us = esp_timer_get_time();
            } else {
                if (!event_has_position) s_miplay_position_ms = frozen_position;
                s_miplay_position_anchor_us = 0;
            }
        }

        if (new_track && !event_has_position &&
            !(event->changed & MIPLAY_MEDIA_CHANGED_PLAYER_STATE)) {
            s_miplay_position_ms = 0;
            s_miplay_position_anchor_us = s_miplay_state == PS_PLAYING ?
                                           esp_timer_get_time() : 0;
        }
        if ((event->changed & MIPLAY_MEDIA_CHANGED_TITLE) && s_miplay_title[0])
            lvgl_port_ui_set_title(s_miplay_title);
        if ((event->changed & MIPLAY_MEDIA_CHANGED_ARTIST) && s_miplay_artist[0] && !lyric_as_title)
            lvgl_port_ui_set_artist(s_miplay_artist);
        if ((event->changed & MIPLAY_MEDIA_CHANGED_COVER_URL) && event->cover_url[0] && !cover_same) {
            s_album_art_gen++;
...
static void dlna_on_miplay_media(const miplay_media_event_t *event)
{
    if (!event || !s_miplay_media_queue) return;
    ESP_LOGI(TAG, "MiPlay media event: changed=0x%08lx title=%.32s artist=%.24s cover=%.96s dur=%lld pos=%lld state=%d",
             (unsigned long)event->changed, event->title, event->artist,
             event->cover_url, (long long)event->duration_ms,
             (long long)event->position_ms, event->player_state);
    if (xQueueSend(s_miplay_media_queue, event, 0) != pdPASS) {
        /* 位置通知可能连续到达，但不能让它覆盖尚未消费的元数据事件。 */
        (void)xQueueReceive(s_miplay_media_queue, &s_miplay_media_drop_work, 0);
        if (xQueueSend(s_miplay_media_queue, event, 0) != pdPASS)
            ESP_LOGW(TAG, "MiPlay media event queue is unavailable");
    }
}
...
static void delayed_stop_notify(void *arg)
{
    ...
    if (gen != s_media_generation) {
        ...
        dlna_delete_current_task(); return;
    }
...
static void cb_set_metadata(const char *metadata)
{
    ...
    if (title[0] && (strcmp(title, s_cur_title) != 0 || strcmp(artist, s_cur_artist) != 0)) {
        ...
        lvgl_port_unlock();
        /* Lyrics: NetEase first, lyrics_fetch falls back to QQ/Kuwo */
        {
            unsigned long netease_id = 0;
            if (custom_dlna_get_music_source() == MUSIC_SRC_NETEASE) {
                netease_id = music_id;
            }
            ESP_LOGI(TAG, "Calling lyrics_fetch_async for: %s - %s (songId=%lu)",
                     s_cur_title, s_cur_artist, netease_id);
            lyrics_fetch_async(s_cur_title, s_cur_artist, netease_id);
        }
    }
}
...
static int decode_base64_cover(const char *source, uint8_t **out_data, int *out_len)
{
    ...
    if (encoded_len < 8 || encoded_len > MIPLAY_COVER_SOURCE_MAX) {
        ESP_LOGW(TAG, "Cover Base64 length invalid: %u (max=%u)",
                 (unsigned)encoded_len, (unsigned)MIPLAY_COVER_SOURCE_MAX);
        return -1;
    }
    ...
    if (ret != 0 || decoded_len < 4) {
        ESP_LOGW(TAG, "Cover Base64 decode failed: ret=%d encoded=%u decoded=%u",
                 ret, (unsigned)encoded_len, (unsigned)decoded_len);
...
static void album_art_task(void *arg)
{
    ...
    while (1) {
        ...
        if (cover_ret != 0 || !img_data || img_len < 4) {
            ESP_LOGW(TAG, "Cover load failed: len=%u %.160s",
    ...
    if (img_data[0] == 0xFF && img_data[1] == 0xD8) {
        ...
        for (int y = 0; y < sh; y++) {
            ...
            for (int x = 0; x < sw; x++) {
                ...
                /* 四个邻近像素（JPEG 解码器输出 RGB565 little-endian） */
    ...
    } else if (img_len >= 12 && memcmp(img_data, "RIFF", 4) == 0 &&
               memcmp(img_data + 8, "WEBP", 4) == 0) {
        ...
        int sw = LVGL_PORT_COVER_SRC_SIZE, sh = LVGL_PORT_COVER_SRC_SIZE;
        size_t out_buf_size = (size_t)sw * (size_t)sh * 2;
        out_buf = (uint16_t *)heap_caps_malloc(out_buf_size, MALLOC_CAP_SPIRAM);
        if (!out_buf) {
            ESP_LOGW(TAG, "WebP out_buf alloc failed (%zu)", out_buf_size);
            WebPFree(webp_rgb);
            free(url);
            continue;
        }
        ...
        WebPFree(webp_rgb);
        out_w = sw; out_h = sh;
        ESP_LOGI(TAG, "WebP decoded: %dx%d -> %dx%d (crop=%d)", webp_w, webp_h, out_w, out_h, crop_sz);
    } else {
        ESP_LOGW(TAG, "Unknown cover format: %02X %02X %02X %02X",
...
static void album_art_worker_init(void)
{
    ...
    if (!s_album_art_queue) {
        ESP_LOGW(TAG, "album_art worker init failed");
        return;
    }
    if (dlna_create_task(album_art_task, "album_art",
                         ALBUM_ART_TASK_STACK_BYTES, NULL,
                         1, &s_album_art_task, 1) != pdPASS) {
...
static void lyrics_btn_setup(void)
{
    ...
    gpio_isr_handler_add(7, btn_lyrics_isr, NULL);
    ESP_LOGI(TAG, "Lyrics button ready (IO7)");
}
...
static void start_dlna(void)
{
    ...
    ESP_LOGI(TAG, "Custom DLNA started (host=ESP-1.85c)");
}
...
static void ui_update_task(void *arg)
{
    ...
    while (1) {
        ...
        bool miplay_active = s_miplay_connected;
        int display_state = miplay_active ? s_miplay_state : (int)get_state();
        int pos_ms = miplay_active ? get_miplay_position_ms() : get_position_ms();
        int pos_sec = pos_ms / 1000;
        int duration_sec = miplay_active ?
                           ((s_miplay_duration_ms + 999) / 1000) : s_dur_cache_sec;
        ESP_LOGD(TAG, "UI: pos=%d dur=%d state=%d source=%s", pos_sec, duration_sec,
        ...
        if (!miplay_active && get_state() == PS_PLAYING && s_dur_cache_sec > 0) {
        ...
        if (!miplay_active && get_state() == PS_PAUSED && !s_user_stopped) {
...
static void wifi_init(void)
{
    ...
    if (wifi_provision_load_sta_config(&wifi_config) != ESP_OK) {
...
static void dlna_on_miplay_connected(bool connected)
{
    ...
    s_miplay_duration_ms = 0;
    s_miplay_position_anchor_us = 0;
    s_miplay_session_reset_pending = true;
...
void app_main(void)
{
    ...
    /* ── 主机名 + WiFi 协议/功耗 ── */
    {
        ...
        if (sta_netif) esp_netif_set_hostname(sta_netif, "ESP-1.85c");
    }
    ...
    esp_err_t tft_ret = tft_init();
    if (tft_ret != ESP_OK) {
        ESP_LOGE(TAG, "TFT initialization failed: %s", esp_err_to_name(tft_ret));
    } else {
        ESP_ERROR_CHECK(lvgl_port_init(5));
    }
    ...
    s_miplay_media_queue = xQueueCreateStatic(
        MIPLAY_MEDIA_QUEUE_LEN, sizeof(miplay_media_event_t),
        s_miplay_media_queue_storage, &s_miplay_media_queue_control);
    if (!s_miplay_media_queue) {
        ESP_LOGE(TAG, "MiPlay media event queue creation failed");
    }

    /* 在启动 MiPlay 网络任务前完成注册，避免握手太快导致首个事件丢失。 */
    miplay_set_connected_cb(dlna_on_miplay_connected);
    miplay_set_media_cb(dlna_on_miplay_media);
